//定义_article_info类，处理文章信息展示界面功能
var _article_info = {};

//是否读取相关文章，0为否，大于0为读取的条数，最大20；
_article_info.related = 0;

//是否读取热门文章，0为否，大于0为读取的条数，最大20；
_article_info.hot = 0;
...
_article_info.related_get = function() {
    if (isNaN(_article_info.related)
            || _article_info.related < 1
            || _article_info.related > 20
            || !_article_info.data['info']
            || !_article_info.data['info']['tags']) {
        return;
    }
...
_article_info.remake = function() {
    if (_article_info.remake_days == 0) {
        return;
    }
...
_article_info.hot_get = function() {
    if (!isNaN(_article_info.hot)
            || _article_info.hot < 1
            || _article_info.hot > 20) {
        return;
    }
    ...
    _wf.ajax({
        'data': {
            'mod': 'article',
            'act': 'article_hot_read',
            'sid': _wf.app.id,
            'show': _article_info.hot
        },
        'func': function(data) {
            _article_info.hot_data = data;
            _article_info.hot_set();
        }
    });
};

_article_info.hot_set = function() {
    if (_article_info.hot_data['num'] == '0') {
        return;
    }

    _article_info.hot_box = $('#hot_box');
    _article_info.hot_dom = $('#hot_box .item:eq(0)').clone(true);
    _article_info.hot_dom_title = _article_info.hot_dom.find('.title');
    _article_info.hot_dom_date = _article_info.hot_dom.find('.date');
    _article_info.hot_dom_info = _article_info.hot_dom.find('.info');

    _article_info.hot_box.html('');
    _article_info.hot_box.css('display', 'block');

    for (i in _article_info.hot_data['list']) {
        _article_info.hot_set_info(_article_info.hot_data['list'][i]);
    }
};

_article_info.hot_set_info = function(info) {
    if (!info['url']) {
        switch (info['xtb']) {
            case 'article' :
                info['url'] = '?name=article_info&xid=' + info['id'];
    ...
    _article_info.hot_dom_title.html(info['label']).attr({'href': info['url']});
    _article_info.hot_dom_date.html(info['ctime']);
    _article_info.hot_dom_info.html(info['info']);
    _article_info.hot_box.append(_article_info.hot_dom.clone(true));
};